...
class OperationsSupaBaseDB {
  ...
  Future<List<dynamic>> getTarefas() async{
    final data = await supabase.from('tarefa').select();
  ...
  Future<void> addTarefa(context, message) async{
    ...
    await supabase.from('tarefa').insert({'id_user': '${user?.id}', 'message': '$message', });
    Navigator.pushReplacementNamed(context, AppRoutes.homePage);
  }

  Future<void> excludeTarefa(context, String id_tarefa) async{
    final User? user = supabase.auth.currentUser;
  ...
  Future<String> editTarefa(context, String id_tarefa, String novo_texto) async{
    final User? user = supabase.auth.currentUser;

    await supabase
        .from('tarefa')
        .update({ 'message': '$novo_texto' })
        .match({ 'id_tarefa': '$id_tarefa', 'id_user': '${user?.id}'});
    Navigator.pushReplacementNamed(context, AppRoutes.homePage);
    return 'sucesso';
  }
